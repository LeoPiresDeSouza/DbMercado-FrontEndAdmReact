...
/**
 * Tema Quartz do admin (catálogo ERP / grids SSRM): escuro, acento azul, painel de colunas refinado.
 * Usado por {@link BaseGrid} com `variant="adminCatalog"` e pelo grid de produtos.
 */
const catalogAccent = '#0091ff';

/** Altura do header — alinhar com `headerHeight` no grid. */
export const CATALOG_GRID_HEADER_PX = 44;

/** Largura padrão do painel lateral Colunas/Filtros. */
export const CATALOG_SIDEBAR_PANEL_PX = 480;

/** Raio dos checkboxes no tema admin (px). */
export const CATALOG_CHECKBOX_BORDER_RADIUS_PX = 3;

const adminCatalogColumnPanelPart = createPart({
  feature: 'dbmercado-admin-catalog-colunas-panel',
  css: () => `
.ag-side-bar,
...
/** Tema Quartz compartilhado — grids administrativos (produtos, logs, etc.). */
export const adminCatalogQuartzTheme = themeQuartz
  .withParams({
  ...
  .withPart(adminCatalogColumnPanelPart);

/** @deprecated Use {@link adminCatalogQuartzTheme}; mantido para imports legados em `modules/products`. */
export const produtosCatalogoQuartzTheme = adminCatalogQuartzTheme;
